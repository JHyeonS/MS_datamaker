...
INVENTORY_CSV = ROOT / "outputs_2s" / "file_inventory.csv"
...
def get_tdms_duration_sec(tdms_path, fs):
    try:
        ...
        raise ImportError("nptdms가 필요합니다. pip install npTDMS") from e
    ...
    if max_len == 0:
        raise RuntimeError(f"TDMS channel length를 읽지 못했습니다: {tdms_path}")
...
def _read_first_trace_n_samples(segy_path):
    ...
    n_samples_trace = _read_be_u2(trace_header, 114)
    return n_samples_trace
...
def get_segy_duration_sec(segy_path, fs=None):
    ...
    if n_samples <= 0:
        raise RuntimeError(f"SEG-Y에서 sample 수를 읽지 못했습니다: {segy_path}")

    if fs is None:
        if info["sample_interval_us"] > 0:
            fs = 1e6 / float(info["sample_interval_us"])
        else:
            raise RuntimeError(f"fs를 추정할 수 없습니다: {segy_path}")

    duration_sec = float(n_samples) / float(fs)
    return duration_sec
...
def get_duration_sec(file_path, site):
    ...
    raise ValueError(f"지원하지 않는 확장자입니다: {file_path}")
...
def make_rows(inv_row, duration_sec, data_type, label, max_segments_per_file):
    ...
    if site == "pohang":
        ...
        for view_name in ["utah_lower", "utah_upper"]:
...
def main():
    ...
    if len(noise_df) > 0:
        ...
        print("[WARN] noise segment가 0개입니다.")
    ...
    if len(unlabel_df) > 0:
        ...
        print("[WARN] unlabel segment가 0개입니다.")
