...
def load_catalog(xlsx_path: str, time_col: str = "UTC") -> pd.DataFrame:
    if xlsx_path.lower().endswith(".csv"):
        read_candidates = [lambda p: pd.read_csv(p)]
    else:
        read_candidates = [
            lambda p: pd.read_excel(p, header=0),   # 일반 xlsx
            lambda p: pd.read_excel(p, skiprows=1)  # 기존 iDAS excel 예외
        ]

    last_cols = None
    for reader in read_candidates:
        df = reader(xlsx_path)
        last_cols = list(df.columns)
        if time_col in df.columns:
            df["event_time_utc"] = pd.to_datetime(df[time_col], utc=True, errors="coerce")
            return df.dropna(subset=["event_time_utc"]).reset_index(drop=True)

    raise ValueError(f"'{time_col}' 컬럼이 없음. 사용 가능한 컬럼: {last_cols}")
