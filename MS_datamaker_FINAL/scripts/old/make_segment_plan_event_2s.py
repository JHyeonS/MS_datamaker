...
UTAH_2019_VIEW1_LABEL_CSV = ROOT / "csv" / "utah_2019_labels_2s_1.csv"
UTAH_2019_VIEW2_LABEL_CSV = ROOT / "csv" / "utah_2019_labels_2s_2.csv"
...
def auto_find_label_col(df):
    preferred = ["is_event", "event", "label", "class", "target", "category", "type", "event_label"]
...
def canonical_site(view_name):
    return "pohang" if view_name == "pohang" else "utah_2019"
...
def make_event_rows(label_df, inventory_df, source_name, view_name):
    ...
    for _, r in label_df.iterrows():
        ...
        if start_col is not None and end_col is not None:
            try:
                seg_start = float(r[start_col]); seg_end = float(r[end_col])
            except Exception:
                continue
...
def main():
    ...
    utah_view1_df = pd.read_csv(UTAH_2019_VIEW1_LABEL_CSV)
    utah_view2_df = pd.read_csv(UTAH_2019_VIEW2_LABEL_CSV)

    all_rows = []
    all_rows.extend(make_event_rows(pohang_df, inventory_df, POHANG_LABEL_CSV.name, "pohang"))
    all_rows.extend(make_event_rows(utah_view1_df, inventory_df, UTAH_2019_VIEW1_LABEL_CSV.name, "utah_2019_1"))
    all_rows.extend(make_event_rows(utah_view2_df, inventory_df, UTAH_2019_VIEW2_LABEL_CSV.name, "utah_2019_2"))
